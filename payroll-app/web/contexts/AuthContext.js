...
// import { checkisLoggedIn, IS_INITIALIZED } from "../flow/scripts";
...
export default function AuthProvider({ children }) {
  // a state variable to keep track of the currentUser
  const [currentUser, setUser] = useState({
    loggedIn: false,
    addr: undefined,
  });
  // a state variable to represent if a user is logged in or not
  const [isLoggedIn, setIsLoggedIn] = useState(false);

  // subscribe to changes in the user (login, logout, etc)
  ...
  //   // Use the `checkisLoggedIn` script we wrote earlier
  //   // and update the state variable as necessary
  //   const checkInit = async () => {
  //     const isInit = await checkisLoggedIn(currentUser.addr);
  //     setIsLoggedIn(isInit);
  //   };
